...
#+EDITED:  2025-10-12; 2025-09-13; 2025-04-12; 2025-03-02; 2025-02-11; 2024-09-04; 2024-02-06; 2024-01-02; 2023-12-10; 2023-09-06; 2023-08-09; 2023-07-12; 2023-04-07; 2023-02-24; 2023-01-03; 2022-12-19; 2022-12-16; 2022-09-25
...
See $HOME/.emacs.d/abbrev_defs where the ~/.emacs.d folder is symlinked to ~/.configure/emacs [Added 2025-04-08]

Abbrevs file is at: $HOME/.config/emacs/abbr_defs

*** abbrevs not working
2025-06-26

Not reading the abbrevs-defs file in ~/.config/emacs/abbrev-defs (which is a symlink from POE)

SOLUTION
M-x read-abbrev-file
~~and enter ~/.config/emacs/abbrev-defs~~
and enter ~/.config/emacs/abbr_defs
(often needed after a system reboot)
...
M-x describe-? v variable; f function ;


C-h i      Info on packages
...
put the link / URI instead of x and the description in the second pair of [].
...
Macros saved in file:
~/.emacs.d/kbd_macros

Can I save my defined macros in a file for persistence?
...
TL;DR
with dashes on items, C-c -

Note:
kmacro: C-c d for enter dash
...
c. OR Shift Right Arrow (Yep. Easy, right? But doesn't always work).
d. OR Ctrl C -
...
Squared       \sup2      backslash sup2

Am I right? ✓

Right arrow \to        backslash (bsl) to

              bsl Gamma                                           bsl pi
Given a circle \Gamma of diameter d, the length of its circumference is \pi{}d.
...
Abbreviations file at:
$HOME/.config/emacs/abbrev_defs
...
->% ls -la
abbrev_defs -> /home/donagh/PORTABLE_ENV/doom/dot_emacs/abbrev_defs
...
bookmark file at $HOME/.config/emacs/.local/etc/bookmark
...
Reading / writing the bookmarks file
After a restart it may be necessary to read the bookmark file:
bookmark file at $HOME/.config/emacs/.local/etc/bookmark

To add a new bookmark to this file; when in the relevant buffer (that you want to bookmark):
M-x bookmark-write
...
** dired
2025-08-11
URL: [[https://discourse.doomemacs.org/t/dired-keybind-reference-sheet/127][dired cheatsheet]]

M-x dired-do-             :: to see much of what dired can do

Entering dired
=================================================================
Evil Key     Vanilla Key   Description
...............................................................
SPC f d      C-c f d       Open directory in dired (prompts you)
SPC o -      C-c o -       Open current directory in dired
SPC p . RET  C-c p D       Open project root in dired
...
[SPC] o -                 :: Dired on current buffer — to, for example, rename file using R in Dired
...
*** URLs_hd
https://emacs.stackexchange.com/questions/19810/move-point-to-previous-next-parent-heading
...
See https://www.emacswiki.org/emacs/CategoryIndentation
and https://www.xemacs.org/Links/tutorials_1.html
...
eg Type in: POUND SIGN --> £
 ...
 EM DASH — (shortcut is lh — with abbrev-mode on )
...
`q` a will begin recording a macro to register `a` and pressing `q` again will stop recording. `@a` will play the macro.
Replace any register with a letter of your choosing.

According to the GNU documentation, you can use kmacro-to-register to store the last recorded macro in a register.
Default keybinding is C-x C-k x r to store in register r.
These registers are not the evil-registers but the emacs registers which can be viewed using M-x view-registers.
...
[^1]: https://www.jamescherti.com/essential-emacs-packages/ some good overview thoughts on essential(ish) emacs packages
...
After pressing <Enter> at the endpoint you'll get
talk to you later
...
See also zim-wiki --> EDITORS:emacs:packages:dired
...
A better alternative is [[vertico]]
...
evil- registers --> :reg OR SPC i r
emacs registers --> M-x view-register
...
Area of circle = \pi r x r OR  \pi.r\sup2


To make that \dagger symbol: M-x org-pretty-mode and <backslash>dagger. See M-x org-entities-help.
Simply in insert mode: <backslash>dagger --> \dag

NOTE: In the following the backslash is omitted to allow the plain text to be visible!
...
SPC b m save current buffer name as a bookmark

M-x bookmark-save --> save current selection of bookmarks to a file located at:
\/home/donagh/.config/emacs/.local/etc/bookmarks

NOTE:
This file-location is symlinked from ~/PORTABLE_ENV/doom/bookmards
...
(("psychology.org"
 (filename . "/run/media/donagh/USB128GB/REPOSITORIES/psychology/psychology.org")
 (front-context-string . "#+TITLE:     psy")
 (rear-context-string)
 (position . 1)
 (last-modified 26718 32714 458784 987000))
("get_living.org"...

** fixing duplicate bookmarks
2025-11-19

M-x bookmark-delete-all
Check that the duplicates are removed from
~/PORTABLE_ENV/doom/bookmarks file (which is linked to the location: ~/.config/emacs/.loca/etc/bookmarks)
using, say, vim to delete the duplicates
Then
M-x bookmark-load
...
[[/home/donagh/files/sample_files/duck.jpg]]
...
** Help
User Manual https://www.gnu.org/software/tramp/#Frequently-Asked-Questions-1
Speed up    https://coredumped.dev/2025/06/18/making-tramp-go-brrrr./
* xHELP
** shortdoc
2025-04-13
