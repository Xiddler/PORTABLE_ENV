...
* DOOM EMACS
** buffers
[SPC] , *OR* [SPC] b b <-- list current buffers
[SPC] <                <-- change buffer
...
** packages
*** meta
REM: Doesn't use emacs packages.el
**** install new package
[SPC] : (or M-x)
Then type: package-install
------
in config.el - (require '<package>) and then C-h r r {Need to check if this works}
in config.el - copy the treemacs line then run doom sync in the /home/donagh/.emacs.d/doom-emacs/bin folder
OR
...
*** dired
**** About
Like Ranger for emacs
**** Commands
New buffer for (current) directory
[SPC] f d
...
*** treemacs
**** About
view directory in a separate panel
**** Commands
[SPC] d
...
**** Commands
gs [SPC] <letter> <-- In-file navigation.
...
- [X] this item [2/2]
  ...
  - [X] Subitem 2
** org-capture
Save notes to [[/home/donagh/Dropbox/org-mode/org/notes.org][notes.org]] using [SPC] X
Then select the appropriate letter - [n] [j] etc.
** search
1. /
